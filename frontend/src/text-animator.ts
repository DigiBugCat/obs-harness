...
export type AnimationStyle = 'typewriter' | 'fade' | 'slide' | 'bounce' | 'wave' | 'none';

// Character state for per-character animations
...
// Wrapped line containing segments (for show() method)
...
// Formatted wrapped line (for streaming)
...
// Committed sentence for streaming display
export interface CommittedSentence {
    lines: FormattedLine[];
}
...
class TextAnimator {
    ...
    private current: AnimationItem | null;
    ...
    private lastRevealTime: number;
    private revealRate: number;
    private streamFadeStart: number | null;
    private streamFadeDuration: number;
    private streamOpacity: number;

    // Max lines to display at once
    private maxDisplayLines: number;
    private hardMaxLines: number;

    // Fade-out state for clearing old sentences
    private clearFadeStart: number | null;
    private clearFadeDuration: number;
    private pendingSentencesToRemove: number | null;

    // Committed sentences (for streaming mode)
    private committedSentences: CommittedSentence[];
    private lastCommittedIndex: number;
    ...
    wrapText(item: AnimationItem): WrappedLine[] {
    ...
    updateTypewriter(item: AnimationItem, elapsed: number): void {
    ...
    updateFade(item: AnimationItem, elapsed: number): void {
    ...
    updateSlide(item: AnimationItem, elapsed: number): void {
    ...
    updateBounce(item: AnimationItem, elapsed: number): void {
    ...
    updateWave(item: AnimationItem, elapsed: number): void {
     ...
     */
    startStream(settings: Partial<StreamSettings>): void {
     ...
     */
    endStream(fadeDelay: number = 500): void {
     ...
     */
    truncateToSpoken(spokenText: string, fadeDelay: number = 1500): void {
        if (!this.isStreaming) return;
     ...
     */
    measureFormattedText(segments: TextSegment[], settings: StreamSettings): number {
        const ctx = this.ctx;
        let width = 0;
     ...
     */
    wrapFormattedParagraph(paraText: string, maxWidth: number, settings: StreamSettings, isQuote: boolean = false): FormattedLine[] {
     ...
     */
    getFormattingStateAt(text: string, position: number): { bold: boolean; italic: boolean } {
        let bold = false;
        let italic = false;
        let i = 0;
     ...
     */
    findSentenceEnd(text: string, startFrom: number = 0): number {
        const endings = ['. ', '! ', '? ', '.\n', '!\n', '?\n', '."', '!"', '?"', ".'", "!'", "?'"];
        let earliest = -1;
        let matchedEnding: string | null = null;
...
if (typeof window !== 'undefined') {
    (window as any).TextAnimator = TextAnimator;
}
